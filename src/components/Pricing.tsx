      ...
      <div className="mt-12 grid grid-cols-1 gap-5 lg:grid-cols-3">
        {tiers.map((t, i) => (
          <motion.div
            ...
            className={cx("card relative flex flex-col p-7", t.highlighted && "border-gold-400/50 shadow-glow lg:-my-3 lg:py-10")}
          >
            {t.highlighted && (
              <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-gradient-to-r from-gold-400 to-gold-600 px-3.5 py-1 text-[10px] font-bold uppercase tracking-wider text-phantix-950">Most popular</span>
            )}
  ...
  return (
    <Section className="pb-28 pt-8">
      <motion.div {...fadeUp} className="relative overflow-hidden rounded-3xl border border-gold-400/25 bg-gradient-to-b from-phantix-800/80 to-phantix-900/80 px-8 py-16 text-center shadow-glow">
        ...
        <h2 className="mt-6 font-display text-4xl font-bold tracking-tight text-white">Take command of your security posture</h2>
        <p className="mx-auto mt-3 max-w-xl text-[15px] text-slate-400">Explore the product on a simulated demo tenant — no account needed. Ready for real work? Register on the Platform and issue login links for your operators to access the Application.</p>
        ...
        <p className="mt-4 text-[11px] text-slate-600">Registration happens on <strong className="text-slate-500">platform.phantix.site</strong>. After setup, you generate login links from the Platform — your team signs in on <strong className="text-slate-500">app.phantix.site</strong>.</p>
      </motion.div>
    </Section>
  );
}
